...
class TransactionController extends Controller
{
    public function addToCart(Request $request)
    {
        ...
        return redirect()->back()->with('status','Car Add to Cart');
    }
    ...
    public function payNow()
    {
       ...
       {
        return redirect()->back()->with('status','Your Wallet is not Sufficient');
       }
       elseif($total_debit == 0)
       {
        return redirect()->back()->with('status','You doesnt have item on cart');
       }
       else{
           ...
           Wallet::create([
            ...
            foreach($carts as $cart){
                if($cart->product->stock > 0){
                    Transaction::where('status', 'on cart')->update(
                        [
                            'status' => $status,
                            'order_id' => $order_id
        ...
        return redirect()->back()->with('status','Succesfullly Payment');
    }
    ...
    public function DeleteCart($id)
     {
        ...
        {
            return redirect('/home')->with('status','"Failed emove Product from the cart."');
        }
     }
    ...
    public function take($id)   
    {
        ...
        return redirect()->back()->with('status','Successfully take');
    }
}
